// shared/combat/combatResolvers.js
import { ARMOR_SLOTS, DAMAGE_TYPES } from "../data/combatTypes.js";
...
export function getWeaponProfile(weapon) {
	...
	return { damageType, primaryStat, flatDamage, statBonus };
}

export function getArmorProfile(armorItem) {
	if (!armorItem) return { slot: null, statBonus: {} };
...
export function getEquippedArmorSlots(equippedArmor) {
	...
	for (const armor of equippedArmor) {
		...
		if (profile.slot && Object.hasOwn(slots, profile.slot)) {
			slots[profile.slot] = { ...armor, ...profile };
		}
	}
...
export function createBuff({ skillId, stat, value, duration }, now = Date.now()) {
	return {
		skillId,
		stat,
		value,
		duration,
		expiresAt: now + (duration || 0) * 1000,
	};
}

export function pruneExpiredBuffs(activeBuffs, now = Date.now()) {
	if (!Array.isArray(activeBuffs)) return [];
	return activeBuffs.filter((buff) => !buff.expiresAt || buff.expiresAt > now);
}
