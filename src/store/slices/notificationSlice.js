    ...
    addNotification: {
      reducer: (state, action) => {
        ...
        if (state.notifications.length > state.maxVisible) {
          ...
          const visibleCount = state.notifications.filter(n => n.visible).length;
          if (visibleCount > state.maxVisible) {
            let hiddenCount = visibleCount - state.maxVisible;
            // Hide oldest notifications
            for (let i = 0; i < state.notifications.length && hiddenCount > 0; i++) {
...
export const selectVisibleNotifications = (state) => 
  state.notifications.notifications.filter(n => n.visible).slice(-state.notifications.maxVisible);
