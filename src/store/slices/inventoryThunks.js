...
export const moveItemBetweenInventories = (fromInventoryId, toInventoryId, itemId, quantity) => {
  return (dispatch, getState) => {
    ...
    if (fromInventoryId === 'player' || state.playerInventory[fromInventoryId]) {
      fromInventory = state.playerInventory[fromInventoryId];
      fromSliceActions = { move: movePlayerItem, add: addPlayerItem, remove: removePlayerItem };
    } else {
      fromInventory = state.placeInventory[fromInventoryId] || 
                     Object.values(state.placeInventory).find(inv => inv.placeId === fromInventoryId);
      fromSliceActions = { move: movePlaceItem, add: addPlaceItem, remove: removePlaceItem };
    }
    ...
    if (toInventoryId === 'player' || state.playerInventory[toInventoryId]) {
      ...
      toSliceActions = { move: movePlayerItem, add: addPlayerItem, remove: removePlayerItem };
    } else {
      toInventory = state.placeInventory[toInventoryId] || 
                   Object.values(state.placeInventory).find(inv => inv.placeId === toInventoryId);
      toSliceActions = { move: movePlaceItem, add: addPlaceItem, remove: removePlaceItem };
    }
    ...
    try {
      // Remove from source
      dispatch(fromSliceActions.move({
        fromInventoryId,
        toInventoryId,
        itemId,
...
export const removeItemFromInventory = (inventoryId, itemId, quantity) => {
  return (dispatch, getState) => {
    ...
    if (inventoryId === 'player' || state.playerInventory[inventoryId]) {
      ...
      inventory = state.placeInventory[inventoryId] || 
                   Object.values(state.placeInventory).find(inv => inv.placeId === inventoryId);
    }
