...
class GameLoop {
  ...
  register(name, updateFn, options = {}) {
    ...
    Logger.log(
      `GameLoop: Registered system: ${name} (priority: ${config.priority}, interval: ${config.interval}ms)`,
      2,
      "game-loop",
    );
  ...
  setEnabled(name, enabled) {
    ...
    if (system) {
      ...
      Logger.log(
        `GameLoop: ${enabled ? "Enabled" : "Disabled"} system: ${name}`,
        2,
        "game-loop",
      );
    }
  ...
  start() {
    ...
    if (typeof requestAnimationFrame !== "undefined") {
      this.frameInterval = requestAnimationFrame((timestamp) =>
