...
export const CombatService = {
  ...
  handlePlayerAttack(enemies) {
    ...
    if (now - (player.lastAttackTime || 0) < (player.attackCooldown || 1000)) return;
    ...
    this.store.dispatch(
      playerDamaged('player','player',targetEnemy.id,damage,'dealt',enemySnapshot.name)
    );
  ...
  checkAllEnemiesDead(placeId) {
    const state = this.store.getState();
    const enemiesInPlace = Object.values(state.enemies.byId).filter(e => e.placeId === placeId);
    const allDead = enemiesInPlace.length > 0 && enemiesInPlace.every(e => e.isDead);
