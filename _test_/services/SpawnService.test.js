...
import SpawnService, { WaveSpawner } from '../../src/game/services/spawnService';
...
describe('SpawnService', () => {
  ...
  beforeEach(() => {
    ...
    enterPlaceHandler = eventBusService.on.mock.calls.find(call => call[0] === 'enterPlace')?.[1];
  });
  ...
  describe('Initialization', () => {
    ...
    it('should setup event listener for enterPlace events', () => {
      expect(eventBusService.on).toHaveBeenCalledWith('enterPlace', expect.any(Function));
    });
  });
  ...
  describe('Place Entry Management', () => {
    it('should create spawner when entering place with spawn config', () => {
      ...
      const enterPlaceHandler = eventBusService.on.mock.calls.find(call => call[0] === 'enterPlace')?.[1];
...
describe('WaveSpawner - Single Enemy Mode', () => {
  ...
  describe('Single Enemy Detection', () => {
    ...
    it('should detect multiple enemy configuration', () => {
      ...
      const multiSpawner = new WaveSpawner('test_place', multiEnemyConfig, eventBusService);
...
it('should handle respawn after enemy death', () => {
      ...
      const initialSpawnCall = eventBusService.emit.mock.calls.find(call => call[0] === 'spawnEnemy');
      const spawnedEnemyId = initialSpawnCall?.[1]?.enemy?.id;
      
      // Simulate enemy death - find enemyDead handler
      const deathCall = eventBusService.on.mock.calls.find(call => call[0] === `enemyDead:test_place`);
...
describe('WaveSpawner - Enemy Catalog Integration', () => {
  ...
  describe('Enemy Creation with Catalog Data', () => {
    it('should use attackDelayRange from enemy catalog', () => {
      ...
      expect(forestBeast.attackDelayRange).toEqual(enemyCatalog.forest_beast.attackDelayRange);

      const woodlandPredator = spawner.createEnemy('woodland_predator');
      expect(woodlandPredator.attackDelayRange).toEqual(enemyCatalog.woodland_predator.attackDelayRange);
    });

    it('should use attackPattern from enemy catalog', () => {
      const forestBeast = spawner.createEnemy('forest_beast');
      expect(forestBeast.attackPattern).toBe(enemyCatalog.forest_beast.attackPattern);
    });
