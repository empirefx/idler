...
describe('inventorySlice reducer and selectors', () => {
  ...
  it('should initialize player inventory with equipment slots', () => {
    const player = state.inventories.player;
  ...
  it('should remove item quantity', () => {
    ...
    expect(newState.inventories.player.items.find(i => i.id === 1).quantity).toBe(3);
  });
  ...
  it('should move items between inventories', () => {
    ...
    expect(newState.inventories.village_center.items.find(i => i.id === 1).quantity).toBe(5);
    expect(newState.inventories.player.items.find(i => i.id === 1).quantity).toBe(10);
  });

  it('should equip and unequip item', () => {
    const equippedState = inventoryReducer(state, equipItem({ inventoryId: 'player', itemId: 'armor1' }));
    expect(equippedState.inventories.player.equipment.body.id).toBe('armor1');
    expect(equippedState.inventories.player.items.some(i => i.id === 'armor1')).toBe(false);
    const unequippedState = inventoryReducer(
      equippedState,
      unequipItem({ inventoryId: 'player', slot: 'body' })
    );
    expect(unequippedState.inventories.player.equipment.body).toBe(null);
    expect(unequippedState.inventories.player.items.some(i => i.id === 'armor1')).toBe(true);
  });

  it('should select inventory by id', () => {
    const selected = selectInventoryById({ inventory: state }, 'player');
    expect(selected).toEqual(state.inventories.player);
  });

  it('should select vault by place id', () => {
    const vault = selectVaultByPlaceId({ inventory: state }, 'village_center');
    expect(vault).toEqual(state.inventories.village_center);
  });
});
